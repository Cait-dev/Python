...
def render_todos():
    ...
    for i in range(0, len(rows)):
        ...
        color = '#dfdfdf' if completed else '#202020'
        l = Checkbutton(frame, text=description,fg=color, width=42, anchor='w', command=complete(id))
        l.grid(row=i, column=0, sticky='w')
        btn = Button(frame, text='Eliminar', command=remove(id), bg='#f44336', fg='white', border='4px')
...
btn = Button(root, text='Agregar', command=addTodo, bg='#4CAF50', fg='white', border='4px')
